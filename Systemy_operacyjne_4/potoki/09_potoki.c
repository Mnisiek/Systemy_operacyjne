...
#define CHUNK_READ 16
#define CHUNK_WRIT 7

int main() {

    char buffer[CHUNK_READ], buffer2[CHUNK_WRIT];
    ...
    while ((bytes_read = read(source, buffer, CHUNK_READ))) {
    ...
    while(( pipe_bytes_read = read(despipe[0], buffer2, CHUNK_WRIT) )) {
